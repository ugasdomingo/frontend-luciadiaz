...
const findRegister = async (uid: any) => {
...
<template>
  <q-card class="my-card" @click="(toggle = !toggle), findRegister(users?._id)">
    <div class="text-subtitle2 text-black">
      <span class="text-subtitle1 q-ml-xl">{{ users?.name }}</span>
      <q-expansion-item>
        <q-card-section v-model="toggle">
          <template v-for="registro of infoRegister" :key="registro.id">
            <AllSelfRegisters :registro="registro" class="q-ma-lg" />
          </template>
        </q-card-section>
      </q-expansion-item>
    </div>
